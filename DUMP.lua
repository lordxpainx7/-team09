...
if not player or not player:FindFirstChild("PlayerGui") then
    warn("ScriptHUB: Falha ao inicializar PlayerGui. Verifique o contexto.")
    return
end
print("ScriptHUB: Inicialização bem-sucedida!")
...
local function showTab(tabName)
    print("ScriptHUB: Mostrando aba " .. tabName)
...
local speed = player.Character and player.Character.Humanoid.WalkSpeed or 16  -- Speed inicial
...
speedSlider.MouseButton1Click:Connect(function()
    speed = speed + 30
    if speed > 4000 then speed = player.Character.Humanoid.WalkSpeed or 16 end
...
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    if aimSilentToggle and getnamecallmethod() == "Raycast" then
        ...
        if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
            args[2] = (target.Character.HumanoidRootPart.Position - args[1]).Unit * (args[3] or 1000)
        end
...
UserInputService.InputBegan:Connect(function(input)
    if flyToggle and (input.KeyCode == Enum.KeyCode.Space or input.UserInputType == Enum.UserInputType.Touch) then
...
RunService.Heartbeat:Connect(function()
    ...
    if flyToggle and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and flyBodyVelocity then
        local moveDirection = Vector3.new()
        if UserInputService:IsKeyDown(Enum.KeyCode.W) then moveDirection = moveDirection + camera.CFrame.LookVector end
        if UserInputService:IsKeyDown(Enum.KeyCode.S) then moveDirection = moveDirection - camera.CFrame.LookVector end
        if UserInputService:IsKeyDown(Enum.KeyCode.A) then moveDirection = moveDirection - camera.CFrame.RightVector end
        if UserInputService:IsKeyDown(Enum.KeyCode.D) then moveDirection = moveDirection + camera.CFrame.RightVector end
        flyBodyVelocity.Velocity = moveDirection.Unit * flySpeed + Vector3.new(0, flyBodyVelocity.Velocity.Y, 0)
    end
end)

-- Infinite Jump
UserInputService.JumpRequest:Connect(function()
    if infiniteJumpToggle and player.Character and player.Character:FindFirstChild("Humanoid") then
...
-- Infinite Ammo
RunService.Heartbeat:Connect(function()
    if infiniteAmmoToggle and player.Character and player.Character:FindFirstChildOfClass("Tool") then
        local tool = player.Character:FindFirstChildOfClass("Tool")
        if tool:FindFirstChild("Ammo") then
            tool.Ammo.Value = tool:FindFirstChild("MaxAmmo") and tool.MaxAmmo.Value or 999
        end
    end
end)
